...
def n19_genx(arr1, arr2)
  ...
  # File.open($FILE, "a") { |f|     f.write("n19_genx crossing Melody ", arr.join, " = ", arr1.join, " x ", arr2.join, "\n")   }
  return arr
end
...
def n19_genplay(melody, sca: scale(:C, :major), info: [])
  # File.open($FILE, "a") { |f| f.write("n19_gen: playing Melody ", melody["mel"].join, " amp:", (melody["amp"].map { |a| a==nil ? "0" : (a*16).to_i.to_s(16) }).join, " in ", sca.to_s[22..-2], "\n" )   }
  info[8]=""
...
with_fx :vowel, mix: 0.2 do
  with_fx :wobble, mix: 0.1 do
    ...
    ctr = synth :pnoise, note: chord(:f, :major), attack_level: 1, attack: 0.5, decay: 4, decay_level: 0.05, sustain: song_len, sustain_level: 0.02, slide: 2, pan: 1
